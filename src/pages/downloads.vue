        ...
        <div class="dl server">
          ...
          <div class="options">
            <div class="os">
              <label>
                Windows
                <input @change="updateServerVersion" type="checkbox" v-model="serverGeneratorForm.isLinux" name="os"> <!-- if checked – linux! !-->
...
export default {
  ...
  methods: {
    updateServerVersion () {
      ...
      return axios.get(`https://cdn.altv.mp/server/${branch}/${target}/update.json`)
        .then(serverInfo => {
    ...
    bundleServer: function () {
      ...
      let files = []
      files.push({
        url: `https://cdn.altv.mp/server/${branch}/${target}/altv-server` + (!this.serverGeneratorForm.isLinux ? '.exe' : ''),
      ...
      if (this.serverGeneratorForm.includeNodeJS) {
        files.push({
          url: `https://cdn.altv.mp/node-module/${branch}/${target}/modules/` + (!this.serverGeneratorForm.isLinux ? 'node-module.dll' : 'libnode-module.so'),
          path: 'modules/' + (!this.serverGeneratorForm.isLinux ? 'node-module.dll' : 'libnode-module.so')
        })

        files.push({
          url: `https://cdn.altv.mp/node-module/${branch}/${target}/` + (!this.serverGeneratorForm.isLinux ? 'libnode.dll' : 'libnode.so.72'),
      ...
      if (this.serverGeneratorForm.includeJS) {
        files.push({
          url: `https://cdn.altv.mp/js-module/${branch}/${target}/modules/js-module/` + (!this.serverGeneratorForm.isLinux ? 'js-module.dll' : 'libjs-module.so'),
          path: 'modules/js-module/' + (!this.serverGeneratorForm.isLinux ? 'js-module.dll' : 'libjs-module.so')
        })

        files.push({
          url: `https://cdn.altv.mp/js-module/${branch}/${target}/modules/js-module/` + (!this.serverGeneratorForm.isLinux ? 'libnode.dll' : 'libnode.so'),
      ...
      if (this.serverGeneratorForm.includeCsharp) {
        files.push({
          url: `https://cdn.altv.mp/coreclr-module/${branch}/${target}/modules/` + (!this.serverGeneratorForm.isLinux ? 'csharp-module.dll' : 'libcsharp-module.so'),
          path: 'modules/' + (!this.serverGeneratorForm.isLinux ? 'csharp-module.dll' : 'libcsharp-module.so')
        })
      ...
      for (const file of files) {
        zip.file(file.path, downloadFile(file.url), { binary: true })
      }
      ...
      new Promise((resolve, reject) => {
          ...
          return downloadFile('https://cdn.altv.mp/samples/resources.zip')
            ...
            .then(resourceZip => {
              ...
              resourceZip.forEach((relativePath, file) => {
                if (!file.dir) {
                  zip.file(relativePath, file.async('blob'), { binary: true })
                }
              })
              resolve()
            })
        ...
        .then(() => {
          return zip.generateAsync({ type: 'blob' }, (metadata) => {
            ...
            .catch(e => {
              this.serverGeneratorForm.isBundling = false
              alert('Failed to bundle server! Please, contact us on Discord! Error: ' + e.message)
            })
        })
    }
  }
...
<style scoped>
  ...
  @media screen and (max-width: 768px) {
    .container, .title p:not(.dlMobile) {
      display: none;
    }
    .dlMobile {
      display: block;
